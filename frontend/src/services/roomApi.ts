import { getHttpBaseUrl } from './collaborationStomp';

export async function roomExistsApi(roomId: string): Promise<boolean> {
  const res = await fetch(`${getHttpBaseUrl()}/api/rooms/${encodeURIComponent(roomId)}/exists`);
...
export async function createRoomApi(roomId: string): Promise<void> {
  const res = await fetch(`${getHttpBaseUrl()}/api/rooms/${encodeURIComponent(roomId)}`, {
    method: 'POST',
  });
...
export async function deleteRoomApi(roomId: string, userId: string): Promise<boolean> {
  const res = await fetch(
    `${getHttpBaseUrl()}/api/rooms/${encodeURIComponent(roomId)}?userId=${encodeURIComponent(userId)}`,
    { method: 'DELETE' }
  );
